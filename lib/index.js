...
function createXaiOAuthAdapter(session, resolveAttachments) {
	return new PiAiAdapter({
		profiles: () => /* @__PURE__ */ new Map([[XAI_OAUTH_ROUTE, {
			...
			maxRequestImageBytes: 419430400,
			requestImagePixelBudget: Number.MAX_SAFE_INTEGER,
			requestImageMaxBytes: 20971520,
...
function withSearchTools(body) {
	...
	const present = new Set(kept.filter(isRecord).map((tool) => tool["type"]).filter((type) => type === "web_search" || type === "x_search"));
